...
> `\d ausleihe`.
>
> <img width="1920" height="1020" alt="DBMS_3" src="https://github.com/user-attachments/assets/6b8733b7-e40f-4b1d-bc42-44ddc9a3a64c" />
...
> *Your answer:*   The order matters because of foreign key dependencies.
exemplar depends on buch, and ausleihe depends on both exemplar and mitglied. Therefore, referenced tables must exist before the tables that reference them can be created.

If you try to create ausleihe first, PostgreSQL will return a foreign key error saying that the referenced tables (e.g. exemplar or mitglied) do not exist.
...
> *Your answer:*  GENERATED ALWAYS AS IDENTITY means that the value of the column is automatically generated by the database and normally cannot be set manually.

If you try to insert a value explicitly (e.g. INSERT INTO mitglied (mitglied_id, ...) VALUES (5, ...)), PostgreSQL will reject it with an error unless you explicitly override the identity behaviour.
...
> *Your answer:*  NUMERIC(6,2) stores exact decimal values, while REAL uses floating-point representation and can introduce rounding errors.

For example, calculating a fee like 0.1 + 0.2 results in:

NUMERIC(6,2) → exactly 0.30
REAL → may result in something like 0.3000000004 due to floating-point precision issues.
